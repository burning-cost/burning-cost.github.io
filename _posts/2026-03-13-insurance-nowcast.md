...
**Package update:** `insurance-nowcast` has been consolidated into [`insurance-monitoring`](/insurance-monitoring/). Install with `uv add insurance-monitoring` — covariate-conditioned IBNR completion is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-monitoring)
</div>

...
description: "Ignoring policy limits and IBNR censoring when fitting tail distributions biases your tail index by ~15%. For UK motor TPBI with xi in the 0.50–0.67 range, even a 15% upward bias in xi materially inflates SCR quantile estimates. insurance-severity already fixes this."
---
...
For UK motor TPBI, where the tail index xi sits in the range 0.50–0.67 (alpha = 1.5–2.0), a 15% upward bias in xi produces an estimate around 0.69 for a true value of 0.60. That gap matters. GPD variance is finite only when xi < 0.5 — at xi = 0.60 the distribution already has infinite variance. A further bias to xi = 0.69 pushes further into infinite-variance territory and the tail quantile estimates diverge sharply at the 99.5th percentile. Your Solvency II SCR is computed off those quantiles. So is your XL layer pricing.

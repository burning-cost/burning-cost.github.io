...
We [wrote previously]({{ site.baseurl }}{% post_url 2026-03-06-spatial-territory-ratemaking-with-bym2 %}) about why k-means territory banding is statistically wrong and what the BYM2 model does instead. That post covers the theory. This one covers the plumbing.
...
The `fix_islands=True` argument handles disconnected components - Scottish island groups, Isles of Scilly, occasionally areas where boundary data has a gap. It connects each isolated node to its nearest node in the main component by Euclidean centroid distance. This is a library-level policy choice: document it in the model documentation.

Queen contiguity means two sectors are adjacent if they share any boundary or vertex. For UK postcode sectors this gives roughly 6 neighbours per sector on average, and around 67,000 edges across the full 11,200-sector graph.
...
If Moran's I is not significant, do not abandon the spatial model - it degrades gracefully to independent credibility weighting as `rho` approaches zero. Note it in the model documentation either way.

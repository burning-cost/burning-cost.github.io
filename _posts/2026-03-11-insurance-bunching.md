...
- [500 Vehicle Makes, One Afternoon, Zero Reproducibility](/2026/03/22/insurance-glm-cluster/) — `insurance-glm-cluster` for factor level banding: the complement to threshold analysis when the problem is too many levels rather than gaming at the boundaries between them.
- [Your Pricing Model Is Discriminating. Here's Which Factor Is Doing It.](/2026/03/10/insurance-fairness-diag/) — `insurance-fairness-diag` for proxy discrimination diagnostics: where bunching detects gaming at pricing thresholds, fairness diagnostics detect when those same factors act as proxies for protected characteristics.
- [Your champion/challenger test has no audit trail](/2026/03/15/your-champion-challenger-test-has-no-audit-trail/) — `insurance-deploy` for structured model deployment: once you have identified and corrected for threshold gaming, getting the updated model into production with sign-off and rollback.

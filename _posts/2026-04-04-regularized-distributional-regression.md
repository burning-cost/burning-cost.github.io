...
tags: [distributional-regression, gamlss, normalizing-flows, regularisation, identifiability, severity, elastic-net, lasso, arXiv-2603.25919, pricing]
...
Theorem 3.1 confirms this: with distinct, non-zero $\lambda_k$ per flow, the penalised objective has a unique minimiser. Identifiability is restored.

This is satisfying because it is not an ad hoc fix. The penalties are doing two things simultaneously: regularising the model (shrinking irrelevant coefficients towards zero, which is useful in its own right for high-dimensional pricing models) and breaking the symmetry that caused the identifiability failure. You are not trading one problem for another.
...
**Oracle property (adaptive Lasso).** The adaptive Lasso variant, which uses weights $w_{k,j} = |\hat{\beta}_{k,j}^{(0)}|^{-\gamma}$ derived from an initial estimate, selects the correct covariate support per flow with probability converging to one. In practice: the model learns which covariates genuinely affect each distributional parameter and which do not, per flow, with asymptotic correctness. This is the property you want when fitting a two-flow severity model and asking "does driving age affect the tail shape or only the body location?"

**Algorithm.** Proximal gradient descent: gradient steps on the smooth log-likelihood component, followed by the proximal operator (soft-thresholding) on the L1 terms. The gradient computation chains through the composition using Jacobians — $\nabla_{\beta_k} \ell$ involves flows $k$ through $K$, not all flows. Cost per iteration is $O(K)$, linear in the number of flows. The naive approach of numerically differentiating through the full composition would be $O(K^2)$; this matters at $K = 3$ or more.
...
Unregularised two-flow model: large coefficients with opposite signs on the same covariates across flows. Standard errors are inflated. The Labbe plots are difficult to interpret because the coefficients are physically implausible. Adding or removing a single covariate causes substantial changes to existing estimates — the hallmark of a poorly identified model.
...
Computational feasibility at insurance scale — 200,000+ claims with $K = 3$ flows — is not benchmarked. The proximal gradient algorithm is $O(K)$ per iteration, which is encouraging, but the number of outer iterations required for convergence on high-dimensional insurance data is unknown.

The oracle property result requires that the initial estimator for the adaptive weights is consistent — which it is, under the paper's conditions. But those conditions include a correctly specified model. For insurance severity, we are always approximating, so the oracle result is guidance about asymptotic behaviour rather than a finite-sample guarantee.

Hyperparameter selection is also unaddressed: flow-specific $\lambda_k$ require tuning, and with $K$ flows the cross-validation grid expands multiplicatively. The paper does not discuss this.

---

## Our Assessment

The identifiability problem is real. Any practitioner who has tried to compose multiple transformation stages — or who has noticed that GAMLSS parameter estimates feel unstable without being able to explain precisely why — will recognise the description immediately. The penalisation fix is the right response, and the oracle property result means you get sparsity per flow as a by-product: automatic selection of which covariates influence each transformation component.
...
Kadhem, S.K. (2026). Regularized Regression by Composition for Distributional Models. arXiv:2603.25919. Submitted 26 March 2026.

Related on Burning Cost:

- [Distributional GBMs for Insurance: Pricing Variance, Not Just the Mean](/2026/03/05/insurance-distributional/) — the insurance-distributional library and why modelling the full distribution matters
- [How Do You Know Your Sigma Model Is Working?](/2026/03/08/validating-gamlss-sigma-models/) — diagnostics for GAMLSS sigma submodels; the instability described here is the first thing to look for

...
[`insurance-interactions`](https://github.com/burningcost/insurance-interactions) automates the search.
...
Here is the concrete failure mode. Suppose the young-driver relativity is 2.3x and the high-performance vehicle group relativity is 1.8x. A multiplicative GLM prices the intersection at 2.3 x 1.8 = 4.14x. The observed frequency at that intersection might be 5.2x the base. You are underpricing by 25%.

The interaction exists in the data. The GBM you ran last year found it - your CatBoost model has a leaf somewhere in its tree structure that captures exactly this. The GLM did not find it because nobody thought to test it systematically, or they tested it and the LR test came back marginal, or the parameter count (10 age bands x 20 vehicle groups = 171 new parameters) looked too expensive without knowing how much deviance it would actually save.
...
The first stage fits a Combined Actuarial Neural Network (Schelldorfer and Wuthrich, SSRN 3320525, 2019) on your GLM's residuals. The architecture uses a skip connection:

```
mu_CANN(x) = mu_GLM(x) * exp(NN(x; theta))
```
...
```
d(i,j) = sum_s  z_s * min(|W1[s,i]|, |W1[s,j]|)
```
...
Read the `n_cells` column alongside `delta_deviance_pct`. The age x vehicle group interaction saves 1.23% of base deviance at a cost of 171 new parameters. The age x NCD interaction saves 0.89% for only 54 parameters. On a credibility argument, that second interaction may be more useful than the first: you are getting three-quarters of the deviance gain at one-third of the parameter cost.
...
**Correlated features.** NID can spread interaction signal across correlated pairs. On UK motor data where age and NCD are correlated above 0.4, activating `mlp_m=True` is not optional - it is required to reduce false positives. Even with MLP-M, treat pairs involving both age and NCD with some scepticism until the A/E plot confirms a genuine non-additive pattern.

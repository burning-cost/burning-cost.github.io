...
Note on model naming: GBZIP (Group-Based ZIP) is the paper's latent-group mixture model described throughout this post. G-ZIGP (Generalised Zero-Inflated Generalised Poisson) is a separate variant that replaces the standard Poisson count component with a Generalised Poisson, adding a dispersion parameter within each group. The two are distinct models; the table reports the G-ZIGP result for comparison but GBZIP is the architecture we focus on here. The G-ZIGP has the best deviance but the worst RMSE. This happens because the generalised Poisson extension improves calibration of the count distribution's shape without necessarily improving mean predictions. The deviance metric rewards distributional fit; RMSE penalises mean prediction error. The GBZIP strikes a better balance for mean prediction purposes, which is what a risk score ultimately needs.

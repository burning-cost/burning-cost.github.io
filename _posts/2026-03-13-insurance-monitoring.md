...
Meanwhile, bodily injury claims for small road traffic accidents have dropped by roughly 50% since the Civil Liability Act 2018 came into force on 31 May 2021. The Official Injury Claim portal and the fixed whiplash tariff have reshaped the BI claims picture fundamentally. The model, trained on a world that no longer exists, is systematically overpricing customers whose risk profile includes higher whiplash exposure. Other segments are being underpriced to compensate. The aggregate A/E washes out to 1.02 because the errors cancel. The PSI is clean because the portfolio composition has not changed — the same customers are buying, just at wrong prices.
...
The critical modification for insurance: weight by exposure, not policy count. A portfolio where young drivers account for 15% of policies but 30% of earned exposure needs the young driver bin's contribution to PSI weighted by their exposure share, not their headcount. Standard PSI from credit scoring does not do this. Our [`insurance-monitoring`](https://github.com/burning-cost/insurance-monitoring) library computes exposure-weighted PSI and CSI throughout.
...
Thresholds: PSI/CSI below 0.10 is stable; 0.10-0.25 warrants investigation; above 0.25 requires action. These originate from 1990s credit scoring practice and are not formally calibrated for insurance, but they remain the industry standard. You can override them per deployment — for a large motor book monitored monthly, tighter bands (0.05/0.15) catch smaller shifts before they compound.
...
The standard alert thresholds from industry practice: A/E outside 0.90-1.10 on a mature accident quarter triggers investigation; A/E outside 0.80-1.20 for two or more consecutive quarters is a model risk event.

One complication that catches teams out: IBNR. Using notified claims as your actuals for recent accident periods systematically understates A/E because claims take 12-18 months to develop. Either restrict your A/E analysis to accident periods that are 12+ months old — which introduces a lag — or apply development factors to project incurred claims to ultimate before running this check.
...
The young driver segment at A/E = 1.16 is the signal you would not see in the aggregate 1.05. That is a meaningful underestimate and warrants investigation before the next rating round.
...
The Gini coefficient — equivalent to 2 * (AUROC - 0.5) for binary outcomes — measures whether your model still rank-orders risks correctly. A Gini of 0.42 at training that has drifted to 0.35 in production is a different problem from a Gini that has held steady at 0.42 while your A/E has drifted from 1.00 to 0.88. One requires a refit. The other requires recalibration.

The key advance in arXiv:2510.04556 (Brauer, Menzel, and Wüthrich, October 2025) is providing an asymptotic normality result for the insurance Gini that enables formal hypothesis testing. Under the null that Gini has not changed, `(G_current - G_reference) / SE` is asymptotically standard normal, where the variance is estimated by bootstrap. This gives you a z-statistic and p-value rather than a subjective judgement about whether the Gini has "changed enough".
...
The three layers are most useful when read together.

**PSI clean, A/E drifted globally, Gini stable:** prior probability shift. Recalibrate the intercept. This is the post-whiplash-reform scenario for BI frequency: claim frequency has dropped, the model's relative ranking is fine, just update the level.
...
**PSI clean, A/E drifted in specific segments only, Gini stable:** local concept drift. The relative risk relationship has changed for a specific segment. Investigate; partial refit or manual override of affected rating factors may be sufficient.

The whiplash scenario falls into the first category. Appropriate response: recalibrate the BI frequency model intercept, document the regulatory trigger (Civil Liability Act 2018), record that refit was not required because discrimination was stable. That is a defensible audit trail under FCA Consumer Duty (PRIN 2A, effective July 2023), which requires firms to evidence that pricing models continue to deliver fair value across customer segments. It is also the kind of documented decision record that PRA SS1/23 model risk governance requires of banks.
...
The annual full model validation is different in character from the routine monitoring. It should include: backtesting on out-of-time data from the most recent policy year; recomputing one-way and two-way analyses to check that rating factor relativities still align with observed loss ratios; and a fresh Gini benchmark to update the baseline against which future quarterly tests are measured. If the model has been recalibrated during the year, the annual validation is also when you document whether the recalibration held across the full period.

---

## The library

[`insurance-monitoring`](https://github.com/burning-cost/insurance-monitoring) implements the three-layer framework:

- Exposure-weighted PSI on model score, CSI with per-feature traffic-light output
- Segmented A/E ratios with exact Poisson confidence intervals
- Gini coefficient with the asymptotic z-test from arXiv:2510.04556
- `MonitoringReport` — orchestrates all checks in a single call with configurable thresholds
- Output as plain dicts or Polars DataFrames, suitable for logging to MLflow or Delta

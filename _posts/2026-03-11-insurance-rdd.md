...
**Package update:** `insurance-rdd` has been consolidated into [`insurance-causal`](/insurance-causal/). Install with `uv add insurance-causal` — regression discontinuity design is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-causal)
</div>

...
PRA SS1/23 — the model risk management supervisory statement, published January 2023 — established the governance expectations for model risk in regulated UK firms: model inventory, tiered oversight based on materiality, independent validation, documented assumptions, monitoring plans, and audit trails. SS1/23 applies to UK banks directly. For insurers, the equivalent obligation comes from Solvency II Articles 120–126 and PRA CP6/24 (the PRA's 2024 consultation on insurance model risk management); for FCA-solo regulated insurers, the SS1/23 framework represents the benchmark that regulators apply in practice when assessing model governance adequacy.

...
**Package update:** `insurance-evt` has been consolidated into [`insurance-severity`](/insurance-distributional/). Install with `uv add insurance-severity` — extreme value theory for large loss pricing is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-severity)
</div>

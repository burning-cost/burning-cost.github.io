...
**Package update:** `insurance-conformal-risk` has been consolidated into [`insurance-conformal`](/insurance-conformal/). Install with `uv add insurance-conformal` — all functionality described here is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-conformal)
</div>
...
[`insurance-conformal`](https://github.com/burning-cost/insurance-conformal) implements this via its `risk` submodule. Three controllers, each targeting a different risk the pricing team cares about.
...
The `insurance-conformal` base module and its `risk` submodule address different questions. They are related but distinct.

The base module controls **coverage probability**: the claim falls within the prediction interval on at least `1 - α` of policies. It finds the smallest interval such that `P(Y ≤ upper) ≥ 1 - α`. The output is an interval. The guarantee is about frequency of inclusion.

The `risk` submodule controls **expected monetary loss**: the expected shortfall from underpriced policies, as a fraction of premium income, is at most `α`. It finds the smallest loading multiplier λ such that `E[max(claim - λ·premium, 0) / premium] ≤ α`. The output is a loading factor. The guarantee is about financial exposure.

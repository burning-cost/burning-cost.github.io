...
The `insurance-credibility` library handles the classical case with the `HierarchicalBuhlmannStraub` class. For the full Bayesian version with PyMC, the pattern above is our recommended starting point — it is the minimum viable model that gives you proper posterior uncertainty and the ability to inject informative priors. The Wüthrich (2025) neural mixed effects extension — replacing the GLM offset with a neural network trained jointly with the random effects — is the research frontier, but the pure-Bayesian version is sufficient for most pricing decisions.

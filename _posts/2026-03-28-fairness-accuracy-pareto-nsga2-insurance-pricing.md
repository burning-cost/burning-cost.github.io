...
The key parameter is the `weights` vector. Weights `[0.4, 0.3, 0.3]` say: "accuracy accounts for 40% of our preference, group fairness 30%, counterfactual fairness 30%." These weights are normalised internally, applied to column-normalised objectives, and appear verbatim in the audit record. One point worth stating explicitly for governance documentation: column normalisation divides each objective by its range across the Pareto front, not by its economic magnitude. The weight vector [0.4, 0.3, 0.3] therefore expresses preference over objective ranges, not absolute importance in economic terms. A governance pack should note this so a regulator does not read the weights as percentage importance scores in an absolute sense.

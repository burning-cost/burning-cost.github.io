...
**Package update:** `insurance-online` has been consolidated into [`insurance-optimise`](https://pypi.org/project/insurance-optimise/). Install with `uv pip install insurance-optimise`  -  bandit pricing algorithms are available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-optimise)
</div>

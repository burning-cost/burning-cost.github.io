...
In July 2021, the IFoA Machine Learning in Reserving Working Party published a tutorial covering GLM, GAM, and XGBoost for general insurance pricing. It is widely referenced in UK actuarial discussions of applied ML for pricing. It is entirely R-based.
...
**Proxy discrimination check.** Before finalising any factor table, run [`insurance-fairness`](https://github.com/burning-cost/insurance-fairness). The FCA's Consumer Duty (PS22/9) requires firms to demonstrate fair value, and the FCA's 2024 Consumer Duty multi-firm review found most insurers' Fair Value Assessments were insufficiently substantiated. Postcode is the factor most likely to be a proxy: Citizens Advice (2022) estimated a £280/year ethnicity penalty in UK motor insurance driven by postcode rating. `insurance-fairness` computes proxy R-squared, mutual information scores, and SHAP-linked price impact, and generates a Markdown audit report mapped to PRIN 2A and the Equality Act s.19.

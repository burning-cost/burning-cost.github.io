...
**Package update:** `insurance-changepoint` has been consolidated into [`insurance-dynamics`](/insurance-monitoring/). Install with `pip install insurance-dynamics` — all functionality described here is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-dynamics)
</div>

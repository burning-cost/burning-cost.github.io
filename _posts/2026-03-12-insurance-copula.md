...
**Package update:** `insurance-copula` has been consolidated into [`insurance-frequency-severity`](/insurance-frequency-severity/). Install with `uv add insurance-frequency-severity` — vine copula dependence modelling is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-frequency-severity)
</div>
...
On Databricks, install pyvinecopulib with `--only-binary=:all:` after the kernel restart that `%uv add insurance-copula[vine]` triggers. Editable installs from `/tmp` do not survive kernel restarts; install from PyPI.

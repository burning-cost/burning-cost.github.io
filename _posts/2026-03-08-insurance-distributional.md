...
**XGBoostLSS** (März, arXiv 1907.03178) supports 26 distributions across XGBoost and LightGBM. Its dependency stack — XGBoost plus PyTorch plus Pyro (Uber's probabilistic programming framework) — is substantial. More critically: it has no Tweedie distribution. Not an obscure gap — Tweedie compound Poisson-Gamma is the standard distribution for motor and home pure premium. XGBoostLSS cannot do your core motor model distributionally.
...
The key output is `volatility_score()` — coefficient of variation per risk:
...
**Safety loading.** `P = mu * (1 + k * CoV)` where k is your risk appetite parameter. Currently you apply the same k to every risk. With per-risk CoV, k becomes optional — the loading already varies. The Risk A / Risk B example from the introduction: if both have mu=350 but Risk B has CoV 40% higher, the correct premium for Risk B is proportionally higher even though the expected loss is identical.

**Underwriter referral rules.** Flag risks where CoV exceeds a threshold. Currently no commercial pricing tool — not Emblem, not Radar, not Guidewire — outputs CoV per risk. The rule "refer any risk with CoV > 0.8" is currently unimplementable in standard pricing workflows.
...
# Check the distribution model is calibrated

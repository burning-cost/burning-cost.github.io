...
We think DML should be the default for any causal question in insurance where treatment was not randomly assigned. That covers telematics scoring, renewal rate sensitivity, channel effects, and campaign evaluation. The library is at [github.com/burning-cost/insurance-causal](https://github.com/burning-cost/insurance-causal). The full benchmark is in `benchmarks/run_benchmark.py`, runnable on Databricks serverless in under two minutes.

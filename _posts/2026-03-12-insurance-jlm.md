...
**Package update:** `insurance-jlm` has been consolidated into [`insurance-telematics`](/insurance-telematics/). Install with `pip install insurance-telematics` — joint longitudinal-survival models are available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-telematics)
</div>

...
This requires your quoting system to log the technical premium separately from the commercial loading at quote time — reconstructing it post-hoc from rate tables is possible but painful, and introduces noise if rate tables have been patched or versioned inconsistently. Many UK teams only log the final commercial price; if you are in that position, the IV approach still works, but the instrument construction step needs careful documentation.
...
FCA Evaluation Paper 25/2 (July 2025) flagged explicitly that elasticity-based pricing requires controls to prevent charging less price-sensitive customers higher prices, including as a proxy for protected characteristics. The paper noted that approximately 1 in 4 motor consumers switched insurer in 2024, up from around 1 in 5 in 2023, and that the PCW channel now accounts for approximately 66% of motor new business (up from around 60% pre-GIPP remedies).

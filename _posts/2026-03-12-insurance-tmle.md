...
**Package update:** `insurance-tmle` has been consolidated into [`insurance-causal`](/insurance-causal/). Install with `uv add insurance-causal` — TMLE doubly robust estimation is available as a submodule. [View on GitHub →](https://github.com/burning-cost/insurance-causal)
</div>

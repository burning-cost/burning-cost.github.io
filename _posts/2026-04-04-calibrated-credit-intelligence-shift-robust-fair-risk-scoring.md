...
Solvency II Articles 120–126 set out the tests and standards for internal model approval: use test (Art. 120), statistical quality standards including uncertainty quantification (Art. 121), calibration standards (Art. 122), P&L attribution (Art. 123), validation standards (Art. 124), and documentation (Art. 125). A Bayesian scorer with a documented ECE of 0.015 is a more defensible position than a point-estimate GBM with post-hoc calibration added as an afterthought.

Consumer Duty (PS22/9) outcome rules and the Equality Act 2010 (section 29 insurance exemption schedule, section 19 indirect discrimination) require firms to ensure pricing does not produce systematically worse outcomes through proxy relationships with protected characteristics. Fairness-constrained training does not replace that monitoring, but it does mean the model has an explicit fairness objective rather than relying entirely on post-hoc adjustment.
...
- [Insurance Model Monitoring in Python: A Practitioner's Guide](/2026/04/04/insurance-model-monitoring-python-practitioner-guide/) — the recalibrate-vs-refit decision framework that the CCI shift-aware layer is automating

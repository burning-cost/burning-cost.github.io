...
We built [`insurance-conformal`](/insurance-conformal/) to apply conformal prediction to insurance pricing models. The key contribution is handling the heteroscedasticity that standard conformal implementations ignore.
...
[`insurance-conformal`](/insurance-conformal/) is on [GitHub](https://github.com/burning-cost/insurance-conformal) and [PyPI](https://pypi.org/project/insurance-conformal/). The library is built around a single entry point - `InsuranceConformalPredictor` - and wraps any sklearn-compatible model. The coverage diagnostics work independently of the predictor via `CoverageDiagnostics` if you have intervals from another source and want to apply the same framework.

...
<script type="application/ld+json">
{
  ...
  "@graph": [
    ...
    },
    {
      ...
      "description": "Model drift detection for deployed pricing — exposure-weighted PSI/CSI, actual-vs-expected ratios, Gini drift z-test. v0.5.0 adds SequentialTest for anytime-valid A/B testing via mSPRT. v0.7.0 adds PITMonitor (anytime-valid calibration change detection via probability integral transforms, Henzi/Murph/Ziegel 2025) and InterpretableDriftDetector (feature-interaction-aware drift attribution with exposure weighting and BH FDR control, based on TRIPODD).",
...
| [insurance-monitoring](https://github.com/burning-cost/insurance-monitoring) | Model drift detection — exposure-weighted PSI/CSI, actual-vs-expected ratios, Gini drift z-test. v0.5.0 adds `SequentialTest` for anytime-valid A/B testing (mSPRT, Johari et al. 2022). v0.7.0 adds `PITMonitor` (anytime-valid calibration change detection via probability integral transforms; Henzi, Murph, Ziegel 2025, arXiv:2603.13156; mixture e-process gives P(sup M_t ≥ 1/α | calibrated) ≤ α for all t — check weekly, monthly, or post-renewal with no correction for repeated testing; complementary to CalibrationChecker which tests absolute calibration, not changes) and `InterpretableDriftDetector` (feature-interaction-aware drift attribution with exposure weighting and Benjamini-Hochberg FDR control across rating factors; tells you which features caused drift, not just that drift occurred; based on TRIPODD, Panda et al. arXiv:2503.06606)<br>_Benchmark: Peeking t-test 25% FPR (5× nominal); mSPRT holds at 1%_ | `pip install insurance-monitoring` |

...
| **Model monitoring** | [`insurance-monitoring`](https://github.com/burning-cost/insurance-monitoring) — exposure-weighted PSI/CSI, A/E ratios, Gini drift z-tests with scheduled alerts. v0.5.0 adds `SequentialTest` for anytime-valid A/B testing (mSPRT; peeking t-test has 25% FPR vs mSPRT at 1%). v0.7.0 adds `PITMonitor` for anytime-valid calibration change detection (probability integral transforms; mixture e-process; Henzi et al. 2025, arXiv:2603.13156) and `InterpretableDriftDetector` for feature-level drift attribution with exposure weighting and Benjamini-Hochberg FDR control across rating factors (based on TRIPODD, Panda et al. 2025, arXiv:2503.06606) | Monitoring dashboards are common in enterprise platforms; insurance-specific metrics vary |

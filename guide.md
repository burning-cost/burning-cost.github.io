...
<tr><td>detect a calibration shift without repeated-test inflation</td><td><a href="https://github.com/burning-cost/insurance-monitoring"><code>insurance-monitoring</code></a> (PITMonitor)</td></tr>
<tr><td>identify which rating factors are driving model drift</td><td><a href="https://github.com/burning-cost/insurance-monitoring"><code>insurance-monitoring</code></a> (InterpretableDriftDetector)</td></tr>
...
<div class="lib-entry">
  ...
  <p class="lib-entry-problem">Your model has been live for nine months and A/E ratios are creeping up — but you do not know whether to recalibrate the intercept, refit with new data, or whether it is just IBNR noise in the most recent quarter. v0.5.0 adds <code>SequentialTest</code> for anytime-valid A/B testing without the peeking problem: a standard t-test run at interim checkpoints has a 25% false positive rate; mSPRT (Johari et al. 2022) with exposure-weighted Poisson holds at 1%. v0.7.0 adds <code>PITMonitor</code> for anytime-valid calibration change detection via probability integral transforms (Henzi, Murph, Ziegel 2025, arXiv:2603.13156): the mixture e-process controls P(sup M_t ≥ 1/α | calibrated) ≤ α for all t, so you can check weekly, monthly, or after every renewal cohort with no correction for repeated testing. Complementary to CalibrationChecker (absolute calibration), not a replacement. Also adds <code>InterpretableDriftDetector</code>: tells you <em>which</em> rating factors caused a drift signal, not just that drift occurred — exposure-weighted Poisson deviance per feature with Benjamini-Hochberg FDR control for testing many factors simultaneously (based on TRIPODD, Panda et al. arXiv:2503.06606).</p>
  <div class="lib-entry-output"><strong>Output:</strong> exposure-weighted PSI/CSI, segmented A/E ratios with IBNR adjustment, Gini z-test with a formal recalibrate-vs-refit decision rule, <code>SequentialTest</code> for sequentially-valid experiment decisions, <code>PITMonitor</code> for anytime-valid calibration monitoring, <code>InterpretableDriftDetector</code> with per-feature attribution and BH-adjusted p-values</div>
</div>

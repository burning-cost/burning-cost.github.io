...
<tr><td>monitor calibration drift with a controlled false positive rate</td><td><a href="https://github.com/burning-cost/insurance-monitoring"><code>insurance-monitoring</code></a> (PITMonitor)</td></tr>
<tr><td>identify which rating factors are causing drift</td><td><a href="https://github.com/burning-cost/insurance-monitoring"><code>insurance-monitoring</code></a> (InterpretableDriftDetector)</td></tr>
...
<div class="lib-entry">
  ...
  <p class="lib-entry-problem">Your model has been live for nine months and A/E ratios are creeping up — but you do not know whether to recalibrate the intercept, refit with new data, or whether it is just IBNR noise in the most recent quarter. v0.5.0 adds <code>SequentialTest</code> for anytime-valid A/B testing of champion/challenger experiments without the peeking problem: a standard t-test run at interim checkpoints has a 25% false positive rate; mSPRT (Johari et al. 2022) with exposure-weighted Poisson holds at 1%. v0.7.0 adds <code>PITMonitor</code> for calibration drift: rather than repeatedly running Hosmer-Lemeshow (which has a 46% false positive rate under random calibration noise), PITMonitor builds an e-process martingale from PIT values and stops only when the evidence is genuine — ~3% FPR in the same conditions, and detects a true 15% drift within roughly 100–200 observation steps. Also in v0.7.0: <code>InterpretableDriftDetector</code> answers the follow-up question — when drift is confirmed, which rating factors are driving it? Interaction-level attribution with Benjamini-Hochberg FDR control: in a 5-factor benchmark with drift planted in 2, both were correctly identified with zero false positives.</p>
  <div class="lib-entry-output"><strong>Output:</strong> exposure-weighted PSI/CSI, segmented A/E ratios with IBNR adjustment, Gini z-test with a formal recalibrate-vs-refit decision rule, <code>SequentialTest</code> for sequentially-valid experiment decisions, <code>PITMonitor</code> for calibration drift with controlled FPR, <code>InterpretableDriftDetector</code> for BH-corrected feature-level attribution</div>
</div>
